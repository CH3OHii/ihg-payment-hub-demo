...
import { alerts, properties, type Alert } from '../../data/mock'
...
export default function AlertsList() {
  ...
  // In Current mode show raw list — lots of red/amber.
  // In Platform mode quiet the list: bulk downgrade most ambers to green (showing remediation complete).
  const displayAlerts = platform
    ? alerts.map((a, i): Alert => (i < 3 ? a : { ...a, severity: 'green', body: a.body + ' · auto-remediated' }))
    : alerts

  const counts = {
    red:   displayAlerts.filter(a => a.severity === 'red').length,
    amber: displayAlerts.filter(a => a.severity === 'amber').length,
    green: displayAlerts.filter(a => a.severity === 'green').length,
  }

  return (
    <>
      <Card
        title="Compliance & Exception Alerts"
        subtitle={`${counts.red} critical · ${counts.amber} warning · ${counts.green} cleared`}
        noPadding
      >
        <ul className="flex flex-col">
          {displayAlerts.slice(0, 8).map(a => (
            <li
              key={a.id}
...
function Dot({ severity }: { severity: Alert['severity'] }) {
  const color =
    ...
    <div className="fixed inset-0 z-40" onClick={onClose}>
      ...
      >
        <header className="px-5 pt-5 pb-3 border-b border-slate-100 flex items-start justify-between">
          <div>
            <div className="text-[10.5px] uppercase tracking-[0.18em] text-slate2">Alert</div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-3 mt-3 text-[12px]">
                <div className="bg-slate-50 rounded-md p-2.5">
                  <div className="text-slate2 text-[10.5px] uppercase tracking-wider">Today's GTV</div>
                  <div className="font-serif text-[16px] text-navy-dark">¥{(property.today_gtv / 1000).toFixed(0)}k</div>
                </div>
                <div className="bg-slate-50 rounded-md p-2.5">
                  <div className="text-slate2 text-[10.5px] uppercase tracking-wider">Compliance</div>
                  <div className="font-serif text-[16px] text-navy-dark">{property.compliance_score}</div>
                </div>
              </div>
            </section>
          )}
        ...
        <footer className="border-t border-slate-100 p-4 flex gap-2 justify-end">
          ...
          <button onClick={onClose} className="px-3 py-1.5 rounded-md text-[12.5px] text-navy-dark border border-slate-200 hover:bg-slate-50">Assign</button>
          <button onClick={onClose} className="px-3 py-1.5 rounded-md text-[12.5px] text-white bg-teal hover:bg-teal-dark">View property</button>
        </footer>
      </aside>
    </div>
  )
